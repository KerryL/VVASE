	...
	const double &y2, double& dxLine, double& dyLine, double& dxEdge, double& dyEdge) const
{
	...
	if (VVASEMath::IsZero(y2 - y1))
	{
		dxLine = 0.0;
		dyLine = halfWidth * VVASEMath::Sign(x2 - x1);

		dxEdge = 0.0;
		dyEdge = (halfWidth + fadeDistance) * VVASEMath::Sign(x2 - x1);
	}
	else if (VVASEMath::IsZero(x2 - x1))
	{
		dxLine = halfWidth * VVASEMath::Sign(y1 - y2);
		dyLine = 0.0;

		dxEdge = (halfWidth + fadeDistance) * VVASEMath::Sign(y1 - y2);
		dyEdge = 0.0;
	}
	else
	{
		...
		dyLine = sqrt(halfWidth * halfWidth / (1.0 + slope * slope)) * VVASEMath::Sign(x2 - x1);
		dxLine = fabs(slope * dyLine) * VVASEMath::Sign(y1 - y2);
