...
void Axis::ComputeGridAndTickCounts(unsigned int &tickCount, unsigned int *gridCount)
{
	...
	{
		assert(majorResolution > 0.0 && VVASEMath::IsValid(majorResolution));
		assert(minorResolution > 0.0 && VVASEMath::IsValid(minorResolution));
...
unsigned int Axis::GetPrecision(void) const
{
	...
	return VVASEMath::GetPrecision(minimum, majorResolution, logarithmic) + 2;
}
