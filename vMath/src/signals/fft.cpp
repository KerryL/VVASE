	...
	const bool &moduloPhase)
{
	...
	if (!moduloPhase)
		VVASEMath::Unwrap(data);

	data *= 180.0 / VVASEMath::Pi;
...
void FastFourierTransform::ApplyHannWindow(Dataset2D &data)
{
	...
	for (i = 0; i < data.GetNumberOfPoints(); i++)
		...
		- cos(2.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1));
}
...
void FastFourierTransform::ApplyHammingWindow(Dataset2D &data)
{
	...
	for (i = 0; i < data.GetNumberOfPoints(); i++)
		...
		* cos(2.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1))) / 0.54;
}
...
void FastFourierTransform::ApplyFlatTopWindow(Dataset2D &data)
{
	...
	for (i = 0; i < data.GetNumberOfPoints(); i++)
		data.GetXPointer()[i] *= 1.0
		- 1.93 * cos(2.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1))
		+ 1.29 * cos(4.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1))
		- 0.388 * cos(6.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1))
		+ 0.032 * cos(8.0 * VVASEMath::Pi * (double)i / double(data.GetNumberOfPoints() - 1));
}
