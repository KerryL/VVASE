...
std::string ExpressionTree::EvaluateExpression(std::string &results)
{
	...
	if (useDoubleStack.top())
		results = wxString::Format("%0.*f",
			VVASEMath::GetPrecision(doubleStack.top(), printfPrecision),
	...
	const Dataset2D &set) const
{
	...
	/*else if (function.CmpNoCase(_T("bit")) == 0)
		return VVASEMath::ApplyBitMask(set, bit);
...
double ExpressionTree::ApplyFunction(const wxString &function, const double &value) const
{
	if (function.CmpNoCase(_T("log")) == 0)
		...
		return atan(value);
	/*else if (function.CmpNoCase(_T("bit")) == 0)
		return VVASEMath::ApplyBitMask(value, bit);*/
...
wxString ExpressionTree::StringAdd(const wxString &first, const double &second) const
{
	return wxString::Format("%0.*f+%s",
		VVASEMath::GetPrecision(second, printfPrecision), second, first.c_str());
}
...
wxString ExpressionTree::StringAdd(const double &first, const wxString &second) const
{
	return wxString::Format("%s+%0.*f", second.c_str(),
		VVASEMath::GetPrecision(first, printfPrecision), first);
}
	...
	const double &coefficient, const int &power) const
{
	...
	{
		if (power == 0)
			expression.Printf("%0.*f", VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient);
		else if (power == 1)
			expression.Printf("%0.*f*s", VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient);
		else
			expression.Printf("%0.*f*s^%i", VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient, power);
	}
	else
	{
		if (power == 0)
			expression.Append(wxString::Format("%+0.*f",
				VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient));
		else if (power == 1)
			expression.Append(wxString::Format("%+0.*f*s",
				VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient));
		else
			expression.Append(wxString::Format("%+0.*f*s^%i",
				VVASEMath::GetPrecision(coefficient, printfPrecision), coefficient, power));
	}
}
